...
namespace fair.h5
{
    public class H5Helper : fair.extensions.shared.SysHelper
    {
        ...
        public override async Task Init()
        {
            ...
            {
                #region 设置客服端配置
                this.ClientConfig = await dataStore.GetConfig<fair.extensions.shared.entity.ClientConfig>("clientConfig");
                if (this.ClientConfig == null && jsRuntime != null) //如果数据库中不存在配置，从JS中读取配置
                {
                    this.ClientConfig = await jsRuntime.InvokeAsync<fair.extensions.shared.entity.ClientConfig>("window.ClientConfig");
