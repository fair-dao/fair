/*fair Js库 */
    export class base {
         ...
         * @param str
         */
        public static getHash(str: string): number {
         ...
         * @param obj  
         */
        public static log(msg: string, ...obj: any[]) { 
            console.log(msg, obj);
        }
         ...
         * @param base64
         */
        public static B64ToBlob(base64: any): Blob {
         ...
         * @param base64
         */
        public static B64ToU8(base64: any): Uint8Array {
            let type = base64.split(",")[0].match(/:(.*?);/)[1];//提取数据类型( image/png等)     
            let bytes = window.atob(base64.split(',')[1]);//提取数据
            let buffer = new ArrayBuffer(bytes.length);//创建数据缓存
            let bts = new Uint8Array(buffer); 
        ...
        //压缩图片
        public static async ZipImg(imgDataUrl: any) {   
      ...
      * @param id  
      */
        public static removeNode(id: any): void {
         ...
         * @param jslist 脚本列表
         * 实例 fairdao.base.loadJs("app","/lib/jquery.js","/lib/aaa.js","alert('ok');");
         */
        public static loadJs(...jss: string[]): any {
        ...
        private static _loadJs(element: any, jslist: string[]): void {
            ...
            try {
                ...
                } else base._loadJs(element, jslist);
            } catch (e) {
                console.error(e);
            }
         ...
         */
        public static CopyData(selector: string): void {
            let element: any;
        ...
        public static GetQuery(url: string, paramName: string): string {
            var p = "?" + paramName + "=";
            var index = url.indexOf(p);
            if (index < 0) index = url.indexOf("&" + paramName + "=");
            if (index >= 0) {
    ...
    export class ui {
        ...
        public static Toast(msg: string, level: string): void {
            ...
            /*自动隐藏信息层*/
            if (len>0) setTimeout(hide, len * 500);
         ...
         */
        public static Message(title: string, msg: string, option: any): any {
            option = option || {};
        ...
        public static Modal(title: string, content: string, buttons: any, option: any): any {
            option = option || {};
         ...
         * 显示或隐藏边栏
         */
        public static SwitchSidebar(state: number) {
            var sidebar: HTMLElement = document.querySelector("aside");
            if (sidebar) {
         ...
         * */
        public static GetElementPoint(el: HTMLElement): any {
        ...
        public static Popup(selector: string, x: number, y: number) {

            var el: HTMLDivElement = document.querySelector(selector);
            var click = function () {
