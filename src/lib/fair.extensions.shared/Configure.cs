...
namespace fair.extensions.shared
{
    public class Configure
    {
        ...
        private static List<fair.extensions.shared.Extender>? extenders;



        private static void ConfigureExtenders(IServiceCollection services, fair.extensions.shared.Extender[]? extenders)
        {
        ...
        static void ProcessAss(Assembly ass)
        {
            ...
            if (name?.StartsWith("fair") == true)
            {
                ...
                if (spaceName == "fair.extensions.shared") return;
                ...
                if (type != null)
                {
                    try
                    {
                        ...
                        // 判断实例化结果是否成功
                        if (instance is fair.extensions.shared.Extender myObject)
                        {
        ...
        public static IServiceCollection ConfigureServices(IServiceCollection services, fair.extensions.shared.Extender[]? extenders)
        {
            ConfigureExtenders(services, extenders);
            fair.extensions.shared.SysHelper.Services = services;
            services.AddTransient<fair.extensions.shared.IAPIHttpClient, fair.extensions.shared.APIHttpClient>();
            ...
            try
            {
                ...
                services.AddSingleton(typeof(fair.extensions.shared.services.TransService));
                #region 本地化
                services.AddLocalization(o => o.ResourcesPath = "resources");
                services.AddSingleton<INewWordRecorder, NewWordRecorder>();
                services.AddSingleton<fair.extensions.shared.localization.GensysLocaler>();
            }
            catch (Exception ex)
            {
